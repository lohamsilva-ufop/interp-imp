...
;       ((compound_stmt NEWLINE) $1))
     ...
     (testlist_list_plus ((tuple_or_test) (list $1))
                          ((tuple_or_test = testlist_list_plus) (eassign $1 $3)))
     ...
     #; (racket_module_name ((string_list_plus)
      ...
      (compound_stmt ((if_stmt) $1))
                   ;  ((while_stmt) $1)
                    ...
                    ; ((funcdef) $1)
                    ; ((classdef) $1))
      (if_stmt ((if test : suite elif_list) (eif $2 $4 $5))
               ;alterar em todos os sentidos
               ((if test : suite elif_list else : suite) (eif $2 $4 $5)))
      (elif_list (() null) 
      ...
      (comparison ((expr) (list $1))
                  ((comparison comp_op expr) ($3 $1 $2)))
      (comp_op ((<) (lambda (x y) (lt x y)))
               ((>) (lambda (x y) (bt x y)))
               ((==)(lambda (x y) (eeq x y)))
               ((>=) (lambda (x y) (bte x y)))
               ((<=) (lambda (x y) (lte x y)))
